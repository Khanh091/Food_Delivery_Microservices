...
export function RestaurantBankAccountsPage() {
  ...
  const [accountToDelete, setAccountToDelete] = useState<RestaurantBankAccount | null>(null)
  ...
  const remove = async () => {
    if (!selectedRestaurant || !accountToDelete) return
    setSaving(true)
    try { await deleteRestaurantBankAccount(selectedRestaurant.id, accountToDelete.id); setAccountToDelete(null); await load(); pushToast('success', 'Đã xóa tài khoản ngân hàng.') }
    catch { pushToast('error', 'Không thể xóa tài khoản ngân hàng.') } finally { setSaving(false) }
  }
  const noRestaurant = restaurants.length === 0 || !selectedRestaurant
  return <div className="owner-page"><RestaurantPageHeader title="Tài khoản ngân hàng" description="Quản lý tài khoản nhận thanh toán của nhà hàng." actions={<Button onClick={() => { setEditor('create'); setFormError(null) }}>Thêm tài khoản</Button>} />
