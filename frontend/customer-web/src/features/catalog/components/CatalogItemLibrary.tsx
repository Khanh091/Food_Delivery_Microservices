...
import { ItemOptionManagerModal } from './ItemOptionManagerModal'
import type { CatalogItem, CatalogItemImage, CatalogItemLibraryItem } from '../types/catalog'

interface CatalogItemLibraryProps {
  restaurantId: string
  refreshKey?: number
  onItemSaved?: (item: CatalogItem, image?: CatalogItemImage) => void
}

export function CatalogItemLibrary({ restaurantId, refreshKey = 0, onItemSaved }: CatalogItemLibraryProps) {
  ...
  const save = async (value: CatalogItemEditorValue) => {
    ...
    try {
      ...
      const image = value.image ? await uploadItemImage(saved.id, value.image) : undefined
      close()
      await load()
      onItemSaved?.(persisted, image)
      pushToast('success', editing ? 'Đã cập nhật món.' : 'Đã tạo món trong thư viện.')
  ...
  return <section className="catalog-library"><div className="catalog-library-header"><div><span className="catalog-eyebrow">Thư viện món ăn</span><h2>Tất cả món của nhà hàng</h2><p>Món vẫn được giữ lại khi gỡ khỏi danh mục và có thể gắn lại vào thực đơn.</p></div><Button icon={<PlusIcon />} onClick={() => { setEditing(null); setFormError(null); setEditorOpen(true) }}>Thêm món</Button></div>
    ...
    <ItemOptionManagerModal open={optionItem !== null} restaurantId={restaurantId} item={optionItem} onClose={() => setOptionItem(null)} />
  </section>
}
