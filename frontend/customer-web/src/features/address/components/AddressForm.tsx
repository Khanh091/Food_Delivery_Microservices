...
const toFormValues = (address?: DeliveryAddress | null): FormValues =>
  address
    ? {
...
export function AddressForm({ address, submitting, submitError, onSubmit, onCancel }: AddressFormProps) {
  ...
  const change = (name: keyof FormValues, value: string | boolean) => {
    setValues((current) => ({
      ...current,
      [name]: name === 'labelType' && value !== 'OTHER' ? value : value,
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const required = [values.recipientName, values.recipientPhone, values.addressLine, values.city]
  ...
  return (
    <form className="address-form" onSubmit={(event) => void submit(event)} noValidate>
      <div className="form-grid two-columns">
        <label>
          Tên người nhận <span aria-hidden="true">*</span>
          <input value={values.recipientName} onChange={(event) => change('recipientName', event.target.value)} maxLength={255} required />
        </label>
        <label>
          Số điện thoại <span aria-hidden="true">*</span>
          <input value={values.recipientPhone} onChange={(event) => change('recipientPhone', event.target.value)} maxLength={20} required inputMode="tel" />
        </label>
      </div>
      ...
      {values.labelType === 'OTHER' && (
        <label>
          Tên địa chỉ <span aria-hidden="true">*</span>
          <input value={values.customLabel} onChange={(event) => change('customLabel', event.target.value)} maxLength={100} required placeholder="Ví dụ: Nhà bố mẹ" />
        </label>
      )}

      <label>
        Địa chỉ <span aria-hidden="true">*</span>
        <input value={values.addressLine} onChange={(event) => change('addressLine', event.target.value)} maxLength={500} required placeholder="Số nhà, tên đường" />
      </label>
      <div className="form-grid three-columns">
        ...
        <label>Tỉnh/thành <span aria-hidden="true">*</span><input value={values.city} onChange={(event) => change('city', event.target.value)} maxLength={255} required /></label>
      </div>
      <div className="form-grid three-columns">
      ...
      <label>
        Ghi chú giao hàng
        <textarea value={values.deliveryNote} onChange={(event) => change('deliveryNote', event.target.value)} maxLength={500} rows={3} placeholder="Ví dụ: Để tại lễ tân, gọi trước khi giao" />
      </label>
      {!address && (
        <label className="checkbox-label">
          <input type="checkbox" checked={values.isDefault} onChange={(event) => change('isDefault', event.target.checked)} />
